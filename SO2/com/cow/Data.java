...
public class Data {
    ...
    public FileDescriptor open(String nombreArchivo) {
        try {
            ...
            RandomAccessFile data = new RandomAccessFile(datatxt, "rw"); // No usar try-with-resources
            ...
            if (!encontrado) {
                ...
                meta.close();
                data.close(); //Cerramos también `data` si no se va a usar
                return null;
            }
            ...
            if (bloques.length == 0) {
                ...
                writeMetadata(nombreArchivo, bloques.length, nuevoBloqueList);
            }
    
            // Crear descriptor
            FileDescriptor descriptor = new FileDescriptor(data, nombreArchivo, bloques, "rw");
            FileDescriptor.filedescriptors.add(descriptor);
    
            System.out.println("Archivo abierto: " + nombreArchivo + ", Bloques: " + Arrays.toString(bloques));
            meta.close(); //Cerramos solo meta, no data
            return descriptor;
    ...
    public void close(String fileName) {
        ...
        for(int i = 0; i < FileDescriptor.filedescriptors.size(); i++){
            if(FileDescriptor.filedescriptors.get(i).getFilename().equals(fileName)){
                descriptorToClose = FileDescriptor.filedescriptors.get(i);
                break;
            }
        }
        ...
        if (descriptorToClose != null) {
            ...
            FileDescriptor.filedescriptors.remove(descriptorToClose); // Eliminar de la lista de descriptores abiertos
